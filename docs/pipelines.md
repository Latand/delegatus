...
A stage attempt reports its completion through the Viewer MCP tool `stage_report`: `{ verdict, findings: [{ severity, text }], summary, stageId? }`, where `severity` is `P0` through `P3`. The server resolves the calling conversation to the live attempt it is running, so a conversation that holds no live attempt, or that names a stage it does not hold, is refused and nothing is written; `stageId` is needed only to disambiguate a conversation holding more than one live stage.
