...
| A deep link (an attention handoff, a notification, a link in a message, a search result) | Its screen is pushed **over the place the operator is**, so Back returns there. A conversation in another project goes over that project's board; the entry the operator left keeps its own project, and Back restores it. |
...
**How the app holds it (#2105).** `src/components/mobile/mobileNav.ts` is the only writer of the phone's history. Each entry carries the whole place — the stack of screens, the sheet, the project it was written on and the fragment it was written for — so a traversal reads where it landed from the entry instead of counting depth, and a reload lands on the same screen. Each screen stands on its own URL: the board on the project's `#p=` (the bare route for the Overview), a conversation on its `#c=` link, a task on `#task=<id>`, a pipeline on `#pipeline=<id>`, the lists on `#pipelines` and `#accounts`; a sheet keeps the URL of the screen under it. The Viewer's own records — a conversation focus (#866) and a project selection — are handed to the store while the phone layout is up (`setFocusHistoryOwner` in `src/lib/navigation/focusHistory.ts`), so a conversation opened from anywhere is one entry and never a focus entry plus a screen above it. The desktop registers no owner and writes its history as before. A screen's scroll and the sections it has open are kept per place in the stack (`useMobileScrollMemory`, `useMobileScreenState`); the board keeps its column and offsets per project (`phoneKanbanPlace.ts`). The browser driver `src/components/mobile/issue1671Evidence.browser.test.tsx` walks it (`-t "#2105"`).
