...
4. **One navigation contract.** Every screen and every sheet over one is one history entry; the browser's back gesture, iOS's edge swipe and the bar's `‹` are the same pop, and Back from a sheet closes it and stays on the screen; a sibling switch (a switcher row, a bar or dock swipe) replaces the top of the stack (§3.3, revised by #2105).
...
| Open a sheet (`⋯`, `⚠`, title cell, ⚙, chip, a card's long press, «Tasks», the search palette) | The sheet opens **over the current screen** and is one history entry of its own, on the screen's URL. A sheet opened from a sheet takes its place. Every screen's `⋯` opens the board menu over that screen; `⚠` opens the queue over that screen. |
| Close a sheet (scrim, `×`, a drag past 80 px, the platform back) | Back to the same screen, scroll position kept, and the sheet's entry goes with it: the next Back leaves the screen. |
| A row in a sheet that opens a screen | The screen takes the sheet's entry, so Back from it returns to the screen the sheet was over. The Tasks list is the one sheet that stays under what its row opens: Back from a task finds the list again. |
| A deep link (an attention handoff, a notification, a link in a message to a conversation, a task or a lane, a search result) | Its screen is pushed **over the place the operator is**, as one entry, so Back returns there. A screen of another project goes over that project's board (over the Overview it simply goes on the stack, drawn by its own project); the entry the operator left keeps its own project, and Back restores it. |
...
**How the app holds it (#2105).** `src/components/mobile/mobileNav.ts` is the only writer of the phone's history. Each entry carries the whole place — the stack of screens, the sheet, the project it was written on and the fragment it was written for — so a traversal reads where it landed from the entry instead of counting depth, and a reload lands on the same screen. Each screen stands on its own URL: the board on the project's `#p=` (the bare route for the Overview), a conversation on its `#c=` link, a task on `#task=<id>`, a pipeline on `#pipeline=<id>`, the lists on `#pipelines` and `#accounts`; a sheet keeps the URL of the screen under it. The Viewer's own records — a conversation focus (#866) and a project selection — are handed to the store while the phone layout is up (`setFocusHistoryOwner` in `src/lib/navigation/focusHistory.ts`), so a conversation opened from anywhere is one entry and never a focus entry plus a screen above it. A component that navigates by the app's own fragment (a ⋯ row that opens the round before a conversation, the superseded banner, a project's archive, a link in a message) asks `navigateToFragment` (`src/lib/navigation/fragmentNavigation.ts`), which the Viewer serves on the phone: a fragment assigned beside the store left a sheet the same tap closed in the history, and Back reopened it. A ‹ taken while the tab stands on an entry no phone wrote (a link that never opened) pops one entry at a time until it lands on the screen under the one it left, since the browser may have put that link's entry above the screen or in its place. A tapped notification's service worker hands its link to the open tab the same way (`public/question-push-sw.js`), so the tab writes the one entry whatever the browser makes of a worker-driven navigation; a tab that does not answer is navigated as before. The desktop registers no owner and writes its history as before. A reload brings a sheet back with its entry. A sheet that needs no choice (⋯, ⚠, the switcher, Tasks) opens again; one that shows what its screen chose to open it on (a card's actions, a lane's menu, a stage's settings, a lane's links) closes, taking its entry, because that choice did not survive (`useSheetSelection`), so no Back is spent on a sheet nobody draws. A reload over the Overview brings the stack back before any answer names the projects its screens belong to; the Overview judges a screen unplaceable (and goes home) only once that answer is in and no link is still resolving. A screen's scroll and the sections it has open are kept per place in the stack (`useMobileScrollMemory`, `useMobileScreenState`); the board keeps its column and offsets per project (`phoneKanbanPlace.ts`). The browser driver `src/components/mobile/issue1671Evidence.browser.test.tsx` walks it (`-t "#2105"`).

Before #2105 a screen push copied whatever URL the tab stood on. After the operator had visited the orchestrator and come back, the board stood on the orchestrator's `#c=` link, a task pushed from there carried it, and Back from that task's agent landed on it: the Viewer's `hashchange` resolver took it for a fresh deep link and opened the orchestrator.
