...
3. **The one loud element is the attention island** (`AttentionIsland`, fenced, untouched). It stays the only warning-coloured thing in the bar. Its mount in `Viewer.tsx` moves from `fixed right-4 top-12` to `top-[10px]` so its 28 px pill centres in the 48 px bar; the bar keeps the existing 236 px right reserve for it. In the document the island follows ⋯ too: the bar's last child is an empty slot (`BarIslandSlot`, `display: contents`, so the row does not move) and on a desktop project leaf the Viewer portals the island's fixed container into it (`BarIslandProvider`), so Tab reaches the island right after ⋯ and before the board body, in the order the bar is read. The Overview and the phone keep the island where it was, ahead of the board.
...
- `Viewer.tsx`: island `top-12` → `top-[10px]`, portaled into the bar's last slot on a project leaf, with the page's text size and leading, which the board's own font would otherwise replace there. No edits under `src/components/attention/`, `src/lib/attention/`, `src/lib/mcp/`, the pipeline data layer, or the limits code.
