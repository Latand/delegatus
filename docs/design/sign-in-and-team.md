...
| # | Decision | Why (short) |
|---|---|---|
| D1 | **Two layers, kept apart: the perimeter and the identity.** The perimeter (`LLV_TOKEN`, `?k=`, the tailnet, Caddy on the stage box) keeps deciding who may reach the Viewer at all. Identity is a new, additive layer that says which **member** a browser is. On a team install a live member session also satisfies the key, and the sign-in surface needs none, so the key never has to go to a teammate (A13). | The perimeter is enforced in `src/proxy.ts` and fed by the runtime host's entries (#1549), which are byte-identical across releases and must stay stable. Replacing it with cookies would touch the host; adding a layer beside it touches nothing there. |
...
| A16a | **Keyless sign-in requests are bounded, and a flood pushes out its own**: at most 4096 device approvals and 4096 Telegram sign-ins that nobody signed in opened may wait at once, per kind (`OPEN_SIGN_IN_REQUEST_LIMIT` in `src/lib/team/members.ts`). A new one always opens; past the bound the waiting request its device polled least recently is deleted, a never-polled one first and the oldest of those first (`evictKeylessChallenges` in `src/lib/team/store.ts`). Every poll that presents a request's proof records the time (`polled_at`), and the sign-in card polls every 3 s. Never deleted: a request a member answered, one a Telegram account pressed Start on (`start_holder`), one a signed-in member or the owner opened, and a recovery link. A passkey sign-in request is not stored at all: its WebAuthn challenge, host and expiry travel in its id under a key the process makes at start, and only a successful sign-in records the challenge, as used (`passkeySignInOptions` in `src/lib/team/passkeys.ts`). A new approval code is checked for a collision through the `user_code` index, and eviction walks the partial index `challenges_keyless_evictable`. | §5.2, §5.3, §5.4 | Anyone who reaches the address can open one, so without a bound a script filled the store within a code's lifetime. Refusing past a bound of 64 let a flood of a few requests a second lock every signed-out member out; dropping the oldest instead let one client push a waiting person's code out in 17 s, well inside the 20-60 s it takes to carry the code to a signed-in device. Evicting by last poll means requests a flood only opens never push out one that is polled; to push a person's out, a flood has to poll 4096 of its own between two polls of that person's card, about 1 400 a second, and past Start nothing a flood does reaches it. Pinning a request by pressing Start takes a Telegram message per request, which Telegram rate-limits. |
...
| A23 | **`agent.stopped` is not written yet.** Interrupt and kill on `/api/proc` and the conversation-host dialog path record nothing in the audit; the action stays in the event vocabulary and its i18n phrase so the record can grow it without a migration. | §3.7, §7.2 | Stopping an agent is a control with several routes (the dialog, `/api/proc`, the runtime interrupt), none of which resolves the acting person today, and "who started which agent" was the ask. Stamping stops belongs with the next slice that touches those routes. |
| A24 | **An event stream a member opened ends with the member's session.** `/api/logs/stream` and `/api/runtime/stream` re-read the session row every 30 s (`sessionBoundStream` in `src/lib/team/streams.ts`) and close once it is revoked, expired or idle, or its member is no longer active; the reconnect then meets the gate. A stream opened by an agent, a Viewer service or a bearer read, and every stream on a solo install, is left as it was. | §4.2, §9 | The gate runs once, when a connection opens, so a revoked member's open tab kept receiving live transcript and runtime events until the connection happened to drop. On a tailnet install without `LLV_TOKEN` that was their only remaining read, and it had no end. |
| A25 | **A queued message is stamped too.** `POST /api/runtime/queue` refuses an anonymous caller in team mode, and for an admitted `add` or `update` records the member under the entry version's delivery key (`<entry>-v<revision>`), the key Codex's delivered record names in its marker. `/api/log/provenance` names that sender under the record's token in `senders`, never in `submissions`, since no outbox row is filed under it. The claim is taken after admission because the host mints the entry id; a replay or a refusal records nothing. | §7.1, A22 | The composer queues a message whenever a Codex agent is busy, which is the usual case, and those messages reached the chat with no sender and left no `message.sent` in the audit. |
| A26 | **The operator spawn capability names a Viewer service.** `teamActor` classifies a request presenting the operator's own spawn capability as `{ kind: "service" }`, as the gate already admits it. Only Viewer processes read that key; no browser holds it. | §3.6 | The scheduled Telegram report launches its agent in process with that capability and no cookie, so on a team install it resolved to `anonymous` and every scheduled report answered 401 before admission. Classifying the capability covers every in-process launcher, whether or not it also sends a service tag. |

**The key is the operator's (§9).** Rule 3 of §4.2 lets a request that
...
|---|---|---|
| `src/proxy.ts` | The perimeter. With `LLV_TOKEN` set, every connection (loopback included, since #1503) presents the token: cookie `llv_auth`, `Authorization: Bearer`, or `?k=` which sets the cookie and redirects. The report frame path is exempt. | Gains the identity gate after it (§4). On a team install a keyless request reaches the sign-in surface, and passes on a live member session (A13). |
...
code is the approval `userCode`, which is voided after 5 wrong entries. As
built, the kinds are `invite`, `approval`, `telegram`, `handoff`, `passkey` and
`recovery`; a passkey sign-in request is not stored until it succeeds, and a
keyless request carries the time its device last polled and the Telegram
account that pressed Start, which decide what a flood may push out (A16a). A
...
1. Exempt paths pass: the sign-in endpoints `/api/team/public`,
   `/api/team/session/{approve,passkey,handoff,sign-out,approval,telegram}`,
   `/api/team/session/{approval,telegram}/<id>` and `/api/team/join/<code>`,
   matched exactly (`SIGN_IN_ENDPOINTS` in `src/lib/team/gate.ts`, so a stray
   path under them stays behind the gate), and `/api/artifact/frame/*`
...
3. A request that authenticated at the perimeter with `Authorization: Bearer`
   ...
   5 at the stamp site), never by the page load. Only the operator holds the
   key on a team install: no link or reply hands it to a teammate (A13).
...
`https://<tailnet host>/join/<handoff code>`, with `?k=<token>` on the
owner's alone (A13). One scan: the perimeter admits `/join/` without the key
(for the owner's link it consumes `?k=`, sets `llv_auth` and redirects to
`/join/<code>`), and the join page redeems a `handoff` challenge by signing the
phone in as that member, whose session is then their way past the perimeter. The
...
someone to "press Start to check the bot" costs nothing. Anyone who reaches
the address can open `/sign-in`, which a team install serves without the key
(A13),, copy the link and send it to the owner. When Start was
...
| Laptop, `--tailscale`: phone opens `https://<host>.ts.net/?k=…`, cookie set, done | Unchanged in solo mode. In team mode the QR carries `/join/<handoff>` (with `?k=…` only on the owner's): the phone is signed in as the scanning member in one scan. |
| Shared box: Caddy password → `?k=` → the app | Unchanged in solo mode. In team mode a teammate needs no key: the Caddy password, then `/sign-in` or their `/join/` link, and their session carries them past the perimeter from then on (A13). |
...
| Two people under one name | A member name is unique across the team, revoked members included, compared without case, spacing or invisible characters, by the full Unicode confusable skeleton of UTS #39 (the generated table in `src/lib/team/confusables.ts`). The skeleton is case sensitive, so each name has two keys, the skeleton of its lowercase and the lowercase of its skeleton skeletonized again, and two names collide when any key of one is a key of the other (`nameKeys` in `src/lib/team/members.ts`). A name someone types (an invite redemption, a rename, a recovery owner, the name the owner gives a Telegram joiner) is refused when taken (409 `name_taken`) and when it mixes letters of two scripts (400 `name_mixed_scripts`, `mixesScripts`; the scripts Japanese, Korean and Chinese write together count as one). A Telegram first name nobody typed that is taken gets a number instead. A colour may repeat; the palette has eight. |
...
- `team.join.title` {inviter} invited you to Delegatus / {inviter} запросив(ла) вас у Delegatus;
  ...
  `team.join.invalidTitle` This link does not work any more / Це посилання вже не діє; `team.join.invalid` It was already used or has expired. Ask the person who invited you for a new one. / Його вже використали, або воно спливло. Попросіть нове в того, хто вас запросив.; below it a secondary `team.join.signIn` Sign in / Увійти to `/sign-in`, because the usual way to land here is a member tapping their old invite;
...
- `team.tabs.members` Members / Учасники; `team.tabs.activity` Activity / Дії; `team.tabs.sessions` Sessions / Сеанси;
  ...
  `team.approve.title` Approve a device / Підтвердити пристрій; `team.approve.code` Code from the other device / Код з іншого пристрою; `team.approve.confirm` Sign in as you {surfaceOn} ({browser})? / Увійти під вашим ім’ям {surfaceOn} ({browser})?, where `team.surfaceOn.*` carries the preposition (on a phone / на телефоні, on a desktop / на комп’ютері, on a tablet / на планшеті, on another device / на іншому пристрої), so no sentence quotes a generic noun as if it were a device name; `team.approve.wrong` That code is not right. / Код неправильний.;
  `team.sessions.thisDevice` this device / цей пристрій; `team.sessions.signOut` Sign out / Вийти (this device's row only); `team.sessions.end` End session / Завершити сеанс (every other row, another person's or another device's); `team.sessions.signOutAll` Sign out everywhere / Вийти всюди; the row's subline puts the status first (active now / активний зараз, or last used {age} / востаннє {age}) and the method after it, and wraps instead of truncating, so a narrow phone never cuts the status; method phrases carry their own preposition: `team.via.claim` when the team was set up / під час створення команди, `team.via.invite` via invite link / через запрошення, `team.via.approval` with approval from another device / з підтвердженням з іншого пристрою, `team.via.telegram` via Telegram / через Telegram, `team.via.passkey` with a passkey / ключем доступу, `team.via.handoff` via phone QR / через QR для телефона, `team.via.recovery` via host recovery / через відновлення з хоста;
  ...
  action phrases `team.action.*` for every `TeamEventAction`: e.g. `message.sent` sent a message / надіслав(ла) повідомлення; `agent.started` started an agent / запустив(ла) агента, with the role in the context column beside the agent's title; `agent.stopped` stopped {role} / зупинив(ла) {role}; `task.created` created a task / створив(ла) задачу; `task.changed.status` moved a task to {to} / переніс(ла) задачу в {to}; `task.changed` edited a task / змінив(ла) задачу; `task.hidden` hid a task / приховав(ла) задачу; `member.joined` joined / приєднався(лась); `member.invited` invited someone / запросив(ла) когось; `member.revoked` revoked {name} / відкликав(ла) {name}; `session.signed_in` signed in {surfaceOn} {via} / увійшов(ла) {surfaceOn} {via}; `session.revoked` ended a session {surfaceOn} / завершив(ла) сеанс {surfaceOn}; `device.approved` approved a sign-in {surfaceOn} / підтвердив(ла) вхід {surfaceOn}; `passkey.added` added a passkey / додав(ла) ключ доступу; `telegram.linked` linked Telegram / прив’язав(ла) Telegram; the remaining actions follow the same pattern.
...
src/lib/team/streams.ts              sessionBoundStream: a member's event stream ends with the session (A24)
