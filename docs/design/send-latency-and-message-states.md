...
The repeated local slow-consumer experiment measured **500.67 ms before** on current main's host and **0.51 ms after** with durable retention; the final run after refreshing main measured **0.98 ms**. Both runs used the same 500 ms consumer and isolated state. These are local append-acknowledgement observations; they do not establish browser paint latency or the later slices' performance targets.
