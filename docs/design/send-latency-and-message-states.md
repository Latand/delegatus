...
These kinds have no orchestration effect in `consumeRuntimeEvent`. The host enqueues their consumption on the existing FIFO before acknowledging the committed journal event. Completion checkpoints, duplicate detection and startup replay retain their existing implementation. A rejected consumer promise keeps the existing diagnostic and recovery behavior.
...
Review identified a retention prerequisite: the bounded journal currently compacts unconsumed events. A blocked consumer plus continued early acknowledgements can therefore lose replay after a crash. The accelerated path must preserve those replay obligations before it is ready to merge. A regression covers a two-event retention window with three output events behind an unrelated consumer. This slice does not establish browser paint latency or the later slices' performance targets.


The smallest retention prerequisite is a durable, targeted consumer hold committed atomically with the event, respected by compaction and released atomically with the consumer checkpoint. The host requests that hold whenever it queues consumer work. It must survive writes during startup before the host constructor runs. An in-memory queue or an acknowledgement backlog limit alone cannot supply that guarantee. Implementation requires `src/runtime-host/journal.ts`, which is outside the assigned slice 1 source fence. Until that prerequisite is authorized and implemented, the two-event crash regression remains failing and the acknowledgement change is blocked from merge.
