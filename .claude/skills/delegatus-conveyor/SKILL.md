---
name: delegatus-conveyor
description: The full issue→implement→review→merge→deploy conveyor for Delegatus — wake loop, lane lifecycle, merge bars, deploy protocol, resource guard, and recovery playbooks. Use when orchestrating multi-lane development on this repo, running long autonomous sessions, or recovering from crashes/stuck reviews. Builds on delegatus-orchestration (spawn/messaging) and review-loop (flow mechanics).
---

# Delegatus conveyor — autonomous multi-lane pipeline

The orchestrator (a long-lived Claude session in `~/.agents/tools/live-log-viewer-next`) drives every open issue through: **issue → worktree lane → implementer agent → review flow → merge on APPROVE → batched deploy → cleanup**. Delegatus owns the clock (#1245): its seat tick checks this project every few minutes and wakes the seat when work is owed, so the seat never schedules itself — no ScheduleWakeup, no CronCreate, no Monitor loop, and a seat still holding one deletes it the moment the mandate arrives. "Every checkpoint" below means every arriving wake, and each wake re-derives board state from bounded snapshots rather than from what the previous one remembered, so the loop survives compaction, host death and rotation.

## Lane lifecycle

1. **Issue** with full spec on GitHub (`agent: *` label names the owner role — runtime comes from the role registry). Bodies with control chars → write file, `--body-file` (or REST `gh api -X POST repos/.../issues -f ...` when GraphQL is rate-limited). The operator's words go into issues and PRs only paraphrased in English, never quoted verbatim: the operator dictates with slang and does not want it quoted.
...
3. **Materialize the implementer as a pipeline stage.** Create the owning pipeline through `POST 127.0.0.1:8898/api/pipelines` with `src` set to the orchestrator transcript and the implementer plus review stages declared before Start. The controller mints the stable client attempt ID, parent edge, and durable pipeline membership. A dead-host recovery may use `retry-stage` after a clean-worktree check or explicit operator approval for its hard reset and clean. Preserve a dirty interrupted worktree in `needs_decision` for the recovery-stage adoption path tracked in #387. A terminal container with a committed head gets an explicit successor pipeline with recovery and review stages; prior attempts remain historical evidence. Prompts stay English; the controller appends the stage_report contract, so a stage prompt names no verdict format of its own. NON-DRAFT PR, typecheck, tests, and scope fences remain part of the stage prompt. Prompt content is role + scope only; model identity and reasoning level are launch parameters. Never name the model or reasoning level in the prompt text.
4. **Run review through the declared pipeline review stage.** The controller binds the flow to the passed implementer transcript. For Codex, the **account-root copy** is the registry head under dual-root duplication; Codex ≥0.144 puts `cwd` directly at `payload.cwd`. Reviewer runtime comes from the role registry; roundLimit 5, mode auto. A 409 `already has an active flow` resolves through the existing flow followed by resume or advance.
...
8. **Deploy in batches** (below), then PATCH Delegatus task cards (`/api/tasks/<uuid>`) — the user tracks progress on the board, never in internal task lists.
...
- **Design-critique gate on all UI/UX**: every visual change is designed or critiqued against rendered screenshots. Reuse or extend the demo-capture renderer (`scripts/demo-capture*`) to capture the feature. Do not interrupt active lanes; critique lands at their finish.
- **UI lane order**: builder → design critique with an `onFail` edge back to the builder → one code review at the final head. The critique owns judgment (weight, wording, hierarchy) and its `fail` verdict returns the lane to the builder; the review runs once, on the head the critique passed. Blocking layout breakage is the reviewer's job too — the standing scaffold rule (`REVIEW_FRAME_RULES`) makes rendered surfaces part of correctness, so evidence that skips a surface the issue names is REQUEST_CHANGES on its own, and overflow, clipped or zero-width controls and overlap are severity-ranked findings with viewport and measured numbers, rendered from an export of the reviewed HEAD (never the live worktree, never the operator's Delegatus).

## Reviewer isolation & delegation depth (#393)

- **Reviewer and verifier roles are terminal.** Each performs every assigned check inside its own single visible session and has zero child-launch capability: no helpers, workflows, teams, swarms, native subagents, Delegatus children, or MCP children — regardless of engine. A reviewer that needs more coverage says so in its verdict; the orchestrator adds a stage.
...
This section, and the tool it names, are about deploying **this** application — Delegatus, the one serving the board — and about no other project's release. The mandate carries none of it (#1760): a manager of somebody else's project reads its own project's release steps, not these.

**The manager's own path (#795, #1321).** YOU decide when to deploy, and you execute it yourself. Your authority is your designated seat, attributed server-side — a session that is not the designated orchestrator is refused, and a seat acts only for its own project, so `deploy_exact_sha` refuses every caller but this project's own seat. Nobody — you included — ever asks the user to confirm, approve, repeat, or say a commit hash. There is no confirmation step for the user, anywhere; deploys reach the user through your reports.
...
3. Topology: runtime-host container owns the **8898 proxy** → promoted `llv-deploy-*` on its candidate port. Nothing else may hold 8898, or the runtime-host crash-loops on bind; a machine that still has a retired systemd unit gets the CLI's migration notice (`docs/docker.md`, "Moving off the systemd install").
...
- `pgrep -c -f "codex exec"` > ~7 → investigate. Cap concurrent codex reviewer rounds at ~2 (brief 3 OK when >10G free).
