...
1. **Issue** with full spec on GitHub (`agent: *` label names the owner model — table in delegatus-orchestration). Bodies with control chars → write file, `--body-file` (or REST `gh api -X POST repos/.../issues -f ...` when GraphQL is rate-limited).
