...
1. **Issue** with full spec on GitHub (`agent: *` label names the owner role — runtime comes from the role registry). Bodies with control chars → write file, `--body-file` (or REST `gh api -X POST repos/.../issues -f ...` when GraphQL is rate-limited).
