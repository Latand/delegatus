...
import { activityResponse, parseActivityQuery, type ActivityResponse } from "@/lib/activity/report";
 ...
 * `range=today|7d|30d`, `tz`, `window` and `break` in minutes, `rounding`.
...
export async function GET(req: NextRequest): Promise<NextResponse<ActivityResponse | ApiError>> {
  ...
  try {
    const body = await activityResponse(parseActivityQuery(req.nextUrl.searchParams));
