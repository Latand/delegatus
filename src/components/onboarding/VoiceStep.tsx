        ...
        {ORDER.map((id) => {
          ...
          const detail = id === "local" && option && !option.available
            ...
            : id === "chatgpt" && option && !option.available
              ? onGoEngines
                ? <button type="button" onClick={onGoEngines} className="self-start rounded-[6px] text-left text-ui text-warning hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/40 max-sm:-my-[13px] max-sm:py-[13px]">{t("onboarding.voice.chatgptNoCodex")}</button>
