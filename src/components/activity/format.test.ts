...
import { agentParts, agentText, approxText, hoursText, nothingRead, partsRound, trustState } from "./format";
...
describe("the trust chip names what was not read", () => {
  ...
  const response = (overrides: { humanMs?: number; requests?: number; complete?: boolean; missingSourceDays?: number; unread?: number[] }): ActivityResponse => ({
    range: { key: "7d", ...window },
    totals: { humanMs: overrides.humanMs ?? 0, requests: overrides.requests ?? 0, coverage: { complete: overrides.complete ?? false, missingHosts: [] }, missingSourceDays: overrides.missingSourceDays ?? 0 },
