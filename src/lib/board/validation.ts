...
function mutation(value: unknown, index: number): BoardMutationV1 {
  ...
  if (raw.kind === "set-presentation") {
    ...
    if (raw.desktopBoard !== undefined && raw.desktopBoard !== null && raw.desktopBoard !== "kanban") throw new ViewValidationError("INVALID_REQUEST", `invalid mutations[${index}].desktopBoard`);
    ...
    return { kind: "set-presentation", ...(raw.viewMode === undefined ? {} : { viewMode: raw.viewMode }), ...(raw.desktopBoard === undefined ? {} : { desktopBoard: raw.desktopBoard as "kanban" | null }), ...(raw.taskPanelOpen === undefined ? {} : { taskPanelOpen: raw.taskPanelOpen }), ...(raw.idleCollapseMinutes === undefined ? {} : { idleCollapseMinutes: raw.idleCollapseMinutes as number | null }) };
  }
