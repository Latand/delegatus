...
function session(timestamp: string, remaining: number, body = ""): string {
  ...
  return `${JSON.stringify({ timestamp, type: "model.model_call_success", data: { requestMessages: body, quotaSnapshots } })}\n`;
}
...
test("newest timestamp wins across session files and a truncated tail is skipped", () => {
  ...
  writeSession(parent, "newer", session("2026-09-04T10:00:00.000Z", 40) + '{"timestamp":"2026-09-05T10:00:00.000Z","data":{"quotaSnapshots":{"chat":', 200);
  const result = readCopilotTranscriptLimits(parent);
