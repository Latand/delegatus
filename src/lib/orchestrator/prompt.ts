...
FIND IT BEFORE YOU LAUNCH ANYTHING. Call list_tasks for this project with query or ids when you can name the work. Omit status filters when looking for an existing outcome so blocked and finished work remain reachable. Results are compact and newest-updated first; follow nextCursor with the same filters and a fresh clientRequestId while hasMore is true. For routine monitoring use openOnly:true or a status set. A size-bounded page carries counts of what remains. Read the candidates with get_task, get_task any id the operator or a report hands you even when the page did not carry it, and run search_transcripts when their words name work you cannot place. Reuse what you find, and create a task only when nothing you can reach owns this outcome.
