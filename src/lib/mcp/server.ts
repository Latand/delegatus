...
const TOOL_DESCRIPTIONS: Record<McpToolName, string> = {
  ...
  account_limits: "Read each account's last observed usage: per account `engine`, `accountId`, `label`, `active`, `fresh` (recent enough for the automatic switch to act on), `plan`, the `session` and `weekly` windows and every metered model tier as {usedPercent, resetsAt}, and `observedAt`. Narrow with `engine` and `accountId`. A read of the durable observations the accounts panel shows; it never asks a provider.",
