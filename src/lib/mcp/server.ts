...
export const TOOL_INPUT_SCHEMAS: Record<McpToolName, z.ZodObject> = {
  ...
  pipeline_action: z.object({
    ...
    stageId: z.string().min(1).optional().describe("The stage a graph edit, a legacy-review conversion or a retry-stage names. retry-stage: the stage the pipeline waits on; its current attempt's launch is filled in when launchId is omitted, and a stage the pipeline no longer waits on is refused."),
    launchId: z.string().min(1).optional().describe("retry-stage only, optional: the launch of the failed attempt being retried, as get_pipeline with stageId answers it. A launch that is no longer the current attempt's is refused."),
