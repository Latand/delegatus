...
| Cancel or change a pending account switch | The prototype's picker offers Cancel switch and a change of target while a conversation's switch waits for its turn | Implemented in K6b (#1705): the picker's Cancel switch, and Change as a cancel followed by the new switch once the cancel is confirmed, over `cancel {expectedRevision}` for a switch whose record is `requested` or `waiting-turn` and `withdraw {operationId}` for one still queued. Only the messages that switch held move back to the conversation's account, with their payload and identity; a cancelled or withdrawn reconfigure never applies, and the queue settles it once. | K6b, when it merges |
| A reseated stage conversation settles on its own verdict | Plan §6.8: a stage conversation switched to another account settles on its own verdict, with no `onFail` activation | Covered in K6b by an engine regression: the source turn ends without a verdict inside the dead-host grace, the stage follows the conversation to its successor generation and passes on the verdict written there, and its fail edge never activates | K6b, when it merges |
