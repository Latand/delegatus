...
| Messages across a completed account switch | Plan §6.8 and the picker: a message sent while a switch waits for its turn is delivered after it | A switch that commits fails every pending delivery of the conversation and empties its text (#1709). The picker says so until it is fixed. | K6c (#1709) |
| A reseated stage conversation settles on its own verdict | Plan §6.8: a stage conversation switched to another account settles on its own verdict, with no `onFail` activation | K6b adds an engine regression only: given the conversation's new path, host availability and the successor's turns, the engine keeps the attempt running across the released host, follows the successor generation and passes on its verdict without the fail edge. No registry migration, commit or delivery runs in it. Nothing starts the successor's turn in production yet, because the message held for it is dropped at commit (#1709), so a verdict-less switched stage would park. The integration test comes with K6c. | K6c (#1709) |
