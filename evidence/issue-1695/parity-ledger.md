...
| Role, engine and model of a waiting stage | The prototype's waiting stage has "Change role or engine…" in its menu and a model pill in its closed composer | The board's waiting stage shows its engine and model. K6a adds its account chip and "Run on account…"; the scheme board's stage placeholder changes the rest today. | A later slice |
| Cancel or change a pending account switch | The prototype's picker offers Cancel switch and a change of target while a conversation's switch waits for its turn | Rolling a switch back through the migration route is undone when the queue retries the `reconfigure` that owns it, and a newer `reconfigure` cancels the messages held for the pending one (#1705). K6a's picker shows the pending switch and says neither is available yet. | K6b, with the fix for #1705 |
| A queued switch's target on another page or after a reload | "Current: account A · Next turn: account B" on every page | A structured switch queued behind a running turn has no migration record yet, and nothing projects its target: only the page that sent it knows it, and it says so. The migration record, once there, shows on every page. | K6b |
| A reseated stage conversation settles on its own verdict | Plan §6.8: a stage conversation switched to another account settles on its own verdict, with no `onFail` activation | No regression test covers a stage conversation's verdict across an account switch yet | K6b |
